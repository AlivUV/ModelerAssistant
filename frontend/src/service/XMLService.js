...
const findUndefinedParticipant = (elements, i) => {
    ...
    if (i < 1)
        elements[i].participant = elements[modifier = i = 1].participant;

    while (!elements[i].participant) {
        elements.participant = elements[i += modifier].participant;
    }
...
const findParticipant = (elements, i, participants) => {
    ...
    return participants.findIndex(e => e.name === elements[i].participant);
}


const findElement = (elementName, participants) => {
    const index = participants.findIndex(participant => participant.elements.find(e => e.name === elementName));
    return [index, participants[index].elements.find(e => e.name === elementName)];
}


export const buildBPMN = ({ participants, elements, flows }) => {
    let bpmn = initBPMN();
    ...
    for (let i = 0; i < elements.length; i++) {
        elements[i] = {
            ...elements[i],
            participant: findParticipant(elements, i, bpmn.participants)
        }
        bpmn = addElement(elements[i], names, bpmn)
    }
...
const newBPMN = {
    ...
    elements: [
        { type: "StartEvent", name: "Inicio" },
        { type: "UserTask", name: "Seleccionar productos", participant: "Cliente" },
        { type: "UserTask", name: "Realizar pedido", participant: "Cliente" },
        { type: "ServiceTask", name: "Procesar pedido", participant: "Tienda en línea" },
        { type: "ServiceTask", name: "Empaquetar productos", participant: "Tienda en línea" },
        { type: "ServiceTask", name: "Programar envío", participant: "Tienda en línea" },
        { type: "UserTask", name: "Confirmar Entrega", participant: "Cliente" },
        { type: "Gateway", name: "Decisión de envío" },
        { type: "ServiceTask", name: "Realizar envío", participant: "Transportista" },
        { type: "EndEvent", name: "Entrega exitosa" },
        { type: "EndEvent", name: "Entrega fallida" }
    ],
    flows: [
        { source: "Inicio", target: "Seleccionar productos" },
        { source: "Seleccionar productos", target: "Realizar pedido" },
        { source: "Realizar pedido", target: "Procesar pedido" },
        { source: "Procesar pedido", target: "Empaquetar productos" },
        { source: "Empaquetar productos", target: "Programar envío" },
        { source: "Programar envío", target: "Decisión de envío" },
        { source: "Decisión de envío", target: "Realizar envío" },
        { source: "Confirmar Entrega", target: "Decisión de envío" },
        { source: "Realizar envío", target: "Entrega exitosa" },
        { source: "Realizar envío", target: "Entrega fallida" },
        { source: "Programar envío", target: "Confirmar Entrega" },
        { source: "Realizar pedido", target: "Seleccionar productos" },
        { source: "Realizar envío", target: "Confirmar Entrega" }
    ]
}
